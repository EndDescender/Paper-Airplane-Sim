...
SWIGINTERN int Swig_var_dyn_set(PyObject *_val) {
  {
    void *argp = 0;
    int res = SWIG_ConvertPtr(_val, &argp, SWIGTYPE_p_PaperAirplaneSimObject,  0  | 0);
    if (!SWIG_IsOK(res)) {
      SWIG_exception_fail(SWIG_ArgError(res), "in variable '""dyn""' of type '""PaperAirplaneSimObject""'");
    }
    if (!argp) {
      SWIG_exception_fail(SWIG_ValueError, "invalid null reference " "in variable '""dyn""' of type '""PaperAirplaneSimObject""'");
    } else {
      PaperAirplaneSimObject * temp;
      temp  = reinterpret_cast< PaperAirplaneSimObject * >(argp);
      dyn = *temp;
      if (SWIG_IsNewObj(res)) delete temp;
    }
  }
...
SWIGINTERN PyObject *Swig_var_dyn_get(void) {
  ...
  pyobj = SWIG_NewPointerObj(SWIG_as_voidptr(&dyn), SWIGTYPE_p_PaperAirplaneSimObject,  0 );
  return pyobj;
}
